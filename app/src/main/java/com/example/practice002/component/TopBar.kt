...
fun TopBarCustom(
    ...
    showSheet: Boolean,
    setSheetState: (sheetState: Boolean) -> Unit,){
    ...
    if (showSheet) {
        BottomSheet(navHost,currentRoute  ,sheetState = sheetState, showSheet = showSheet, onDismissRequest = { setSheetState(false) }, onDrag = {
            setSheetState(false)
        },
            { setSheetState(!showSheet) })
    }
